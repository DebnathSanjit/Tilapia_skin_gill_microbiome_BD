...
# After plotting the barplot several times, I think plotting alphabetically 
# is much better to interpret between different phyla  
...
#ps <- readRDS("phyloseq_FSS_BD_metadata_v7_16S_abd30_20240703.rds")
ps <- readRDS("phyloseq_FSS_BD_metadata_v7_16S_abd30_20241016.rds")
ps # 33312 taxa and 334 samples

# For now, just consider tilapia and remove other fish
# tax_fix as it necessary for microbViz
pseq <- ps %>%
...
# There are group_by, facet_by and average functions from microViz package, 
## which can easily plot. Two problems which i couldn't solve are, 1) They
### consider the whole dataset to see the top phyla. So top 10 phyla in gill
#### and skin might be same but not in the water. So, instead of using these
##### functions directly, I need to make subset, plot separate plot for each 
# sample type, and combine together. 2) 
...
# To calculate the top 10 phyla, if I consider the whole phyloseq object 
## and also if I split the disease and non-diseased, it doesn't change the top 10 phyla
### But since I'm not only considering disease and non-diseased but sample types,
#### Top 10 phyla of water is different that skin and gill.
### So, I'll use this one but I need to subset disease and non-diseased, 
#### otherwise, when I plot, all the ponds are plotted
...
## legend are not common is diseased and non-diseased. So add one now and then add the remainng taxa
...
## edit in inkscape. to remove seperate border and put a single border for 
